boves : 8 observations (periods) and 13 variables (ceramic types) --> only 4 empty cells, not fit for analysis
(NO)

chevelon : 12 observations (sites) and 10 variables (ground stone types)
(YES)

compiegne : 5 observations (periods) and 16 variables (ceramic types) --> only 3 empty cells, not fit for analysis
(NO)

loire : 332 observations (assemblages) and 331 variables (ceramic types) --> site, city, area
(YES)*3

merzbach : 8 observations (chronological phases) and 36 variables (pottery motifs)
(YES)

mississippi : 20 observations (assemblages) and 10 variables (ceramic types) --> not fit for analysis ?
(NO)

zuni : 420 observations (assemblages) and 18 variables (ceramic types) --> not fit for analysis ?
(NO)